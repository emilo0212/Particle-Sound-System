...
//Privat constructor til partiklernes opdaterings funktion og tegnings funktion
...
//Initialiseringen af SDL til at skabe vinduet. 
void SDLMain::Init(const char* TITLE, int XPOS, int YPOS, int WIDTH, int HEIGHT){
	...
	//Brugergr�nsefladen bliver initialiseret.
	imguiinit->Init(renderer);
}

//SDL Handleevents, som klarer de forskellige events i programmet, som opdateringen af musseposition
//opdateringen af taster, om vinduet k�rer, om det er lukket osv.
...
//SDL opdaterings funktionen som k�rer partiklernes opdaterings funktion.
void SDLMain::Update(){
	pConstruction(0, renderer);
}

//Tegningen af SDL, som ogs� tegner brugergr�nsefladen (imgui) og partiklerne.
void SDLMain::Render(){
...
//Clean funktionen, som sletter alt ved nedlukning af programmet.
void SDLMain::Clean(){
